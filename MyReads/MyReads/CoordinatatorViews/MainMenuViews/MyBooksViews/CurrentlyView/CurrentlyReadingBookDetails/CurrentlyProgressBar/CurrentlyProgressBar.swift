...
struct CurrentlyProgressBar: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(alignment: .center ,spacing: 15) {
                ...
                if let pageCount = book.pageCount, pageCount > 0 {
                    ProgressView(value: Double(book.pagesRead), total: Double(pageCount))
                    
                    let percentage = (Double(book.pagesRead) / Double(pageCount)) * 100
                    
                    Text("\(Int(percentage))%").font(.system(size: 14)).foregroundStyle(.white)
                    
                } else {
                    Text("No data")
                        .font(.system(size: 14)).italic().foregroundStyle(.colorAccentOrange)
                }
            }
                    ...
                    Text("Update")
                        ...
                        .background {
                            RoundedRectangle(cornerRadius: 8)
                                .fill(.colorbackground6.opacity(0.3))
                        }
                })
            }
        ...
        .sheet(isPresented: $showUpdateProgress) {
            CurrentlyUpdateProgress(userProfileViewModel: userProfileViewModel, book: $book)
