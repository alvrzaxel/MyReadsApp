...
//  MyReads
//
//  Created by Axel Álvarez Santos on 2/8/24.
//
...
struct CurrentlyView: View {
    ...
    @State var currentlyReadsBooks: [UserBookModel] = []
    
    var body: some View {
        ScrollView(.horizontal) {
            HStack(spacing: 10) {
                ForEach(currentlyReadsBooks.indices, id: \.self) { index in
                    ZStack {
                        CurrentlyBackground(userProfileViewModel: userProfileViewModel, book: $currentlyReadsBooks[index])
                        
                        CurrentlyBookDetails(userProfileViewModel: userProfileViewModel, book: $currentlyReadsBooks[index], onDismiss: {
    ...
    private func updateCurrentlyReadingBooks() {
...
#Preview("ReadingView") {
    ...
    return CurrentlyView(userProfileViewModel: user)
}
