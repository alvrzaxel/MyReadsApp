...
struct BookDetailsDescripcion: View {
    var bookDescripcion: String?
    
    var body: some View {
        if let descripcion = bookDescripcion {
            VStack(alignment: .leading, spacing: 20) {
                HStack {
                    Spacer(minLength: 0)
                    
                    Text("Desciption".uppercased())
    ...
    func formatText(_ text: String) -> String {
        ...
        // Añadir salto de línea después de `»` (solo cuando se encuentra `»`)
        formattedText = formattedText
            .replacingOccurrences(of: "». ", with: "».\n\n")
        
        // Eliminar posibles saltos de línea redundantes si `»` está al final del texto
        formattedText = formattedText
