//
//  BookDetailsImageBacground.swift
//  MyReads
//
...
struct BookDetailsImageBacground: View {
            ...
            VStack {
                if let imageUrl = book.imageLinks?.thumbnail {
                    AsyncImageView(urlString: imageUrl)
                        .scaledToFill()
                        .frame(width: geometry.size.width + 50, height: 550)
                        .clipped()
                        .blur(radius: 20)
                        .offset(x: -25) // Mueve la imagen hacia la izquierda para que se extienda más allá del borde izquierdo
                } else {
                    Rectangle()
                        .frame(width: geometry.size.width + 50, height: 250)
                        .foregroundStyle(.customOrange6)
                        .clipped()
                        .blur(radius: 20)
                        .offset(x: -25)
                }
