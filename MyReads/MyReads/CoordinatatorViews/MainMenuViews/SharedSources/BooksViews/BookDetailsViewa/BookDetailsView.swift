...
struct BookDetailsView: View {
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                BookDetailsDescripcion(bookDescripcion: book.description)
            }
            
            VStack(alignment: .leading) {
                BookDetailsImageBacground(book: book)
                    .overlay(alignment: .top) {
                        BookDetailsHeader(userProfileViewModel: userProfileViewModel, book: $book, bookStatus: $bookStatus)
                    }
        ...
        .overlay(alignment: .topTrailing) {
            Button(action: {
                ...
                Image(systemName: "xmark")
                    .font(.system(size: 24, weight: .bold)).foregroundStyle(.textNegroBlanco)
                    .padding(6)
                    .background(.backgroundGeneral, in: .circle).opacity(0.5)
            })
            .padding()
        }
