...
struct BookDetailsResume: View {
    ...
    var body: some View {
        RoundedRectangle(cornerRadius: 10)
            .foregroundStyle(.thinMaterial.opacity(0.3))
            .frame(maxWidth: .infinity, maxHeight: 100)
            .padding(.horizontal)
            .padding(.top)
            .shadow(color: .backgroundGeneral, radius: 1)
            .overlay {
                ...
                VStack(spacing: 13) {
                    ...
                    HStack(alignment: .center) {
                        ...
                        Spacer(minLength: 0)
                        VStack {
                            Text(String(book.myRating)).font(.system(size: 14, weight: .semibold))
                            Text("My Rating".uppercased()).font(.system(size: 8))
                        }
                        Spacer(minLength: 0)
                        ...
                        Spacer(minLength: 0)
                        VStack {
                            Text(String(book.pageCount ?? 0)).font(.system(size: 14, weight: .semibold))
                            Text("Pages".uppercased()).font(.system(size: 8))
                        }
                        ...
                        Spacer(minLength: 0)
                        VStack {
                            Text(book.language?.uppercased() ?? "UNK").font(.system(size: 14, weight: .semibold))
                            Text("language".uppercased()).font(.system(size: 8))
                        }
                        Spacer(minLength: 0)
                    }
...
struct BookDetailsActions: View {
    ...
    func colorForStatus(_ status: BookStatus) -> Color {
        switch status {
        case .wantToRead:
            return .green.opacity(0.3)
        case .read:
            return .customOrange6.opacity(0.5)
